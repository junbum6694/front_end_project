...
function renderVideoCard(video, options = {}) {
  const { target, type = "card" } = options;
  ...
  <div class="card h-100" data-id="${video.id}">
    <div class="thumbnail-wrapper position-relative w-100">
      <div class="ratio-wrapper" style="padding-top: 56.25%; position: relative;">
        <img class="thumbnail position-absolute top-0 start-0 w-100 h-100"
          src="${video.thumbnail}" style="object-fit: cover;" />
      </div>
    </div>
    <div class="card-body d-flex">
      <div class="img-box">
        <img src="../images/${video.author}.png" width="40" height="40"
          class="d-inline-block align-text-top me-3 rounded-circle" />
      </div>
...
// 모든 카드 공통 클릭 이벤트
$(document).on("click", ".card, .side-list-container", function () {
